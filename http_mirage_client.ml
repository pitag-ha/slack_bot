...
let prepare_http_1_1_headers headers host user_pass body_length =
  ...
  let headers =
    ...
    | None -> headers
    | Some v -> add headers "content-length" (string_of_int v)
  in
...
let prepare_h2_headers headers host user_pass body_length =
  ...
  let headers =
    add headers "content-length"
      (string_of_int (Option.value ~default:0 body_length))
  in
  add_authentication ~add headers user_pass
  ...
  let error_handler e =
    let err =
      ...
      | `Protocol_error (err, msg) ->
          ...
          Format.kfprintf kerr Format.str_formatter "%a: %s"
            H2.Error_code.pp_hum err msg
      | `Exn e -> Error (`Msg ("Exception here: " ^ Printexc.to_string e))
    in
    wakeup err
  in
...
let single_request ~ctx ~alpn_protocol ?config cfg ~meth ~headers ?body uri =
  ...
  | (Some `HTTP_1_1 | None), Some (`HTTP_1_1 config) ->
      single_http_1_1_request ~sleep ~config flow user_pass host meth path
        headers body
  ...
  | (Some `H2 | None), Some (`H2 config) ->
      single_h2_request ~sleep ~config ~scheme flow user_pass host meth path
        headers body
  | Some `H2, None ->
      single_h2_request ~sleep ~scheme flow user_pass host meth path headers
        body
...
let one_request ?config ?tls_config:cfg ~ctx ~alpn_protocol ~authenticator
    ?(meth = `GET) ?(headers = []) ?body ?(max_redirect = 5)
